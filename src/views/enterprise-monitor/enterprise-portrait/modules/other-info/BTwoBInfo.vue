    ...
    <table class="project-table">
      ...
      <tr class="project-table-tr">
        ...
        <td class="project-value">
          {{ infoData?.miaoShuMark }}<br />{{ infoData?.miaoShuMarkComp }}
        </td>
        <td class="project-name">服务态度<br />(同行业相比)</td>
        <td class="project-value">
          {{ infoData?.fuWuMark }}<br />{{ infoData?.fuWuMarkcomp }}
        </td>
      </tr>
      <tr class="project-table-tr">
        <td class="project-name">到货速度<br />(同行业相比)</td>
        <td class="project-value">
          {{ infoData?.daoHoMark }}<br />{{ infoData?.daoHoMarkComp }}
        </td>
...
export default {
  ...
  methods: {
    ...
    getVoluntaryProductsData() {
      ...
      this.$axios
        .post("/riskManager/other/b2bInfo", params)
        .then(({ data }) => {
